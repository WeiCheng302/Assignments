...
foldername = 'Reflect_Result_With_Roll'
homedir = os.getcwd()
...
for fname in filenames:	
	...
	with open(fname[0:-4]+'_Reflect.csv', 'r', newline='') as infile2:
		...
		rows.insert(2, column="mean", value=rows["Cali_Reflect"].rolling(rolling_size).mean().mean())
		rows.insert(3, column="plus_std", value=rows["Cali_Reflect"].rolling(rolling_size).mean()+rows["Cali_Reflect"].rolling(rolling_size).mean().std())
		rows.insert(4, column="minus_std", value=rows["Cali_Reflect"].rolling(rolling_size).mean()-rows["Cali_Reflect"].rolling(rolling_size).mean().std())
		...
		plt.plot(rows["wavelength(nm)"], rows["Cali_Reflect"].rolling(rolling_size).mean(),c = "r", linewidth=1.5)
		...
		plt.plot(rows["wavelength(nm)"][rows["Cali_Reflect"].rolling(rolling_size).mean().idxmax()],rows["Cali_Reflect"].rolling(rolling_size).mean().max(),'^')
		
		#Draw chart
		show_max = ' ('+str(rows["wavelength(nm)"][rows["Cali_Reflect"].rolling(rolling_size).mean().idxmax()])+', '+str('%.3f'%(rows["Cali_Reflect"].rolling(rolling_size).mean().max()))+')'
		plt.annotate(show_max,xytext=(rows["wavelength(nm)"][rows["Cali_Reflect"].rolling(rolling_size).mean().idxmax()],rows["Cali_Reflect"].rolling(rolling_size).mean().max()),xy=(rows["wavelength(nm)"][rows["Cali_Reflect"].rolling(rolling_size).mean().idxmax()],rows["Cali_Reflect"].rolling(rolling_size).mean().max()))
		plt.legend(labels=[fname[0:-4],'mean','plus_std','minus_std',fname[0:-4]+'_max'], loc = 'best')
...
for fname in filenames:	
	...
	with open(fname[0:-4]+'_Reflect.csv', 'r', newline='') as infile3:
		...
		#Plot lines
		plt.plot(df['wav' + fname[0:-4]], df['Reflect' + fname[0:-4]].rolling(15).mean(),c = color[count], linewidth=1.5)
		plt.plot(df['wav' + fname[0:-4]][df['Reflect' + fname[0:-4]].rolling(15).mean().idxmax()], df['Reflect' + fname[0:-4]].rolling(15).mean().max(),'^')
		
		show_max = ' ('+str(df['Reflect' + fname[0:-4]].rolling(15).mean().idxmax())+', '+str('%.3f'%( df['Reflect' + fname[0:-4]].rolling(15).mean().max()))+')'
		plt.annotate(show_max,xytext=(df['wav' + fname[0:-4]][df['Reflect' + fname[0:-4]].rolling(15).mean().idxmax()], df['Reflect' + fname[0:-4]].rolling(15).mean().max()),xy=(df['wav' + fname[0:-4]][df['Reflect' + fname[0:-4]].rolling(15).mean().idxmax()], df['Reflect' + fname[0:-4]].rolling(15).mean().max()))
		plt.xlabel("Wavelength(nm)", fontweight = "bold")
		plt.ylabel("Reflection_Rate", fontweight = "bold")
	...
	with open(fname[0:-4]+'_Reflect.csv', 'r', newline='') as infile4:
		...
		for item in df2['wav' + fname[0:-4]]:
			...
			if 458>float(item)>396 :				
				sum_396_458.append(df2['Reflect' + fname[0:-4]].rolling(15).mean()[idx])
			elif 586>float(item)>506:
				sum_506_586.append(df2['Reflect' + fname[0:-4]].rolling(15).mean()[idx])
			elif 694>float(item)>624:
				sum_624_694.append(df2['Reflect' + fname[0:-4]].rolling(15).mean()[idx])
			elif 749>float(item)>699:
				sum_699_749.append(df2['Reflect' + fname[0:-4]].rolling(15).mean()[idx])
			elif 901>float(item)>765:
				sum_765_901.append(df2['Reflect' + fname[0:-4]].rolling(15).mean()[idx])
			
			if 515>float(item)>442:
				sum_442_515.append(df2['Reflect' + fname[0:-4]].rolling(15).mean()[idx])
			elif 632>float(item)>584:
				sum_584_632.append(df2['Reflect' + fname[0:-4]].rolling(15).mean()[idx])
			elif 1043>float(item)>856:
				sum_856_1043.append(df2['Reflect' + fname[0:-4]].rolling(15).mean()[idx])
